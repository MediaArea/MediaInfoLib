...
void File__Analyze::Get_DolbyVision_ReferenceProcessingUnit(DV_RPU& data)
{
    ...
    int16u active_area_left_offset{}, active_area_right_offset{}, active_area_top_offset{}, active_area_bottom_offset{};
    ...
    data.noise_reduction = noise_reduction;
    data.mpeg_noise_reduction = mpeg_noise_reduction;
    data.frame_rate_conversion = frame_rate_conversion;
    data.brightness = brightness;
    data.color = color;
    if (vdr_rpu_profile == 0 && BL_video_full_range_flag) {
        data.profile_deduced = 5;
    }
    if (vdr_rpu_profile == 1) {
        if (el_spatial_resampling_filter_flag && !disable_residual_flag) {
            data.profile_deduced = 7;
        }
        if (!el_spatial_resampling_filter_flag) {
            data.profile_deduced = 8;
        }
    }
    FILLING_END();
}
