...
void File_Tiff::Streams_Finish()
{
    if (Retrieve_Const(Stream_General, 0, General_Format).empty()) {
        ...
        else if (FileExt == __T("nef") || FileExt == __T("NEF"))
            Fill(Stream_General, 0, General_Format, "NEF");
        else if (FileExt == __T("nrw") || FileExt == __T("NRW"))
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (Info != Infos[i].end())
        {
            ...
            Fill(Stream_Image, StreamPos_Current, Image_Format, Tiff_Compression(Value));
            Fill(Stream_Image, StreamPos_Current, Image_Codec, Tiff_Compression(Value));
