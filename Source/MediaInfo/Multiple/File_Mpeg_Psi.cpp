...
void File_Mpeg_Psi::Table_4E()
{
    ...
    while (Element_Offset<Element_Size)
    {
        ...
        FILLING_BEGIN();
            ...
            eventItem.start_time.From_UTF8("UTC "+Date_MJD(date)+' '+Time_BCD(time));
...
void File_Mpeg_Psi::Table_70()
{
    ...
    FILLING_BEGIN();
        if (Complete_Stream->Duration_Start.empty())
            Complete_Stream->Duration_Start.From_UTF8("UTC "+Date_MJD(date)+' '+Time_BCD(time));
        Complete_Stream->Duration_End.From_UTF8("UTC "+Date_MJD(date)+' '+Time_BCD(time));
