...
void File_Exif::FileHeader_Parse()
{
    ...
    if (IsMakernote) {
        if (Element_Size >= 10 && !strncmp((const char*)Buffer + Buffer_Offset, "Apple iOS", 10)) { // the char* contains a terminating \0
            int16u alignment;
            ...
            currentIFD = Kind_MakernoteApple;
            SkipHeader = true;
            IFD_Offsets[Buffer_Offset + Element_Offset] = currentIFD;
        }
        else if (Element_Size >= 7 && !strncmp((const char*)Buffer + Buffer_Offset, "Nikon\0\2", 7)) {
            Skip_String(7,                                      "Identifier");
            Skip_XX(3,                                          "(Unknown)");
            currentIFD = Kind_MakernoteNikon;
            OffsetFromContainer = 10;
        }
        else if (Element_Size >= 12 &&
            (!strncmp((const char*)Buffer + Buffer_Offset, "SONY DSC \0\0", 12)) || // the char* contains another terminating \0
            (!strncmp((const char*)Buffer + Buffer_Offset, "SONY CAM \0\0", 12)) || // the char* contains another terminating \0
            (!strncmp((const char*)Buffer + Buffer_Offset, "SONY MOBILE", 12))      // the char* contains another terminating \0
            ) {
            Skip_String(12,                                     "Identifier");
            currentIFD = Kind_MakernoteSony;
            OffsetFromContainer = -static_cast<int64s>(MakernoteOffset);
            SkipHeader = true;
            IFD_Offsets[MakernoteOffset + Buffer_Offset + Element_Offset] = currentIFD;
        }
        else {
            --Element_Level;
